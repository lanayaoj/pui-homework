...
/* Add the roll to the cart */
function addToCart() {
    const glazePrice = parseFloat(glazeSelect.value);
    const packSize = packSelect.value;
    const totalPrice = (currentRoll.basePrice + glazePrice) * parseInt(packSize);
    
    const cartRoll = addNewRoll(
        rollType,
    ...
    console.log(`Added to cart: ${cartRoll.type}, Price: $${cartRoll.totalPrice.toFixed(2)}`);
}
